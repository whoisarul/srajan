...
CREATE TABLE public.organic_solutions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    name TEXT NOT NULL,
...
CREATE INDEX idx_tasks_crop_id ON public.tasks(crop_id);
CREATE INDEX idx_tasks_due_date ON public.tasks(due_date);
CREATE INDEX idx_land_analyses_user_id ON public.land_analyses(user_id);
CREATE INDEX idx_land_analyses_land_id ON public.land_analyses(land_id);
CREATE INDEX idx_crop_plans_user_id ON public.crop_plans(user_id);
CREATE INDEX idx_growth_records_crop_id ON public.growth_records(crop_id);
